...
void EventLoopThreadPool::start(const ThreadInitCallback& cb) { // 在 TcpServer::start() 中调用
    assert(!started_);
    ...
    // 如果numThreads_为0，则在baseLoop_线程中调用cb（默认为 0, cb 默认也为空）
    if (numThreads_ == 0 && cb) {
        cb(baseLoop_);
    }
}
...
EventLoop* EventLoopThreadPool::getNextLoop() {
    ...
    // 如果有其他线程，则采用round-robin方式分配（默认无）
    if (!loops_.empty()) { // loops_不为空，说明有其他线程
        // round-robin
        loop = loops_[next_];
        ++next_;
