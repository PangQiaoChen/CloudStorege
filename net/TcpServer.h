...
private:
    ...
    std::shared_ptr<EventLoopThreadPool> threadPool_; // 网络 IO 线程池【默认为0，即不开启额外的 IO 子线程，仅由单线程处理】
    ...
    std::atomic_bool started_;                 // 服务器是否已启动【初始未启动】
    int nextConnId_;                           // 下一个连接ID【初始为0】
