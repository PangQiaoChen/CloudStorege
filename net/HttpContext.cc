  ...
  if (space != end && request_.setMethod(start, space))
  {
    ...
    if (space != end)
    {
      ...
      if (question != space)
      {
        ...
        request_.setQuery(question, space); // 请求方法 /user/login?username=tom&password=123 中的 username=tom&password=123
      }
      else
      {
...
HttpContext::ParseResult HttpContext::parseRequest(Buffer* buf, Timestamp receiveTime) {
    ...
    while (hasMore) {
        if (state_ == kExpectRequestLine) { // 首次默认状态，期待请求行
            const char* crlf = buf->findCRLF(); // 查找请求行结束位置
            if (crlf) {
        ...
        } else if (state_ == kExpectHeaders) { // 请求行解析成功，期待请求头
            ok = processHeaders(buf);
            if (ok) {
                if (state_ == kExpectBody) { // 请求头解析完成，期待请求体
            ...
            if (processBody(buf)) {
                state_ = kGotAll;
                result = kGotRequest;
            } else if (bodyReceived_ < contentLength_) { // 可能一次 TCP 包没有接收完整个请求体
                // 还需要更多数据
                result = kHeadersComplete;
            }
            hasMore = false;
