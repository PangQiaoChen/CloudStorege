...
Acceptor::Acceptor(EventLoop* loop, const InetAddress& listenAddr, bool reuseport)
    : loop_(loop),  // 记录所属的事件循环
      acceptSocket_(sockets::createNonblockingOrDie(listenAddr.family())),  // 创建并封装非阻塞 socket
      acceptChannel_(loop, acceptSocket_.fd()),  // 仅初始化接受通道
      ...
      idleFd_(::open("/dev/null", O_RDONLY | O_CLOEXEC))  // 打开空闲文件描述符,用于防止文件描述符耗尽
{
    ...
    acceptSocket_.bindAddress(listenAddr);  // 调用 bind() 接口绑定监听地址
