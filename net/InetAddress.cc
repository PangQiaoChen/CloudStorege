...
InetAddress::InetAddress(uint16_t port, bool loopbackOnly)
{
    ...
    // 决定 IP 是 127.0.0.1 还是 0.0.0.0
    in_addr_t ip = loopbackOnly ? kInaddrLoopback : kInaddrAny; // 监听所有网卡 默认 kInaddrAny-INADDR_ANY
    addr_.sin_addr.s_addr = sockets::hostToNetwork32(ip); // ip（主机字节序->网络字节序）
    addr_.sin_port = sockets::hostToNetwork16(port); // 端口（主机字节序->网络字节序）
}
