...
class HttpRequest : public mymuduo::copyable
{
 public:
  ...
  {
    string result;
    std::map<string, string>::const_iterator it = headers_.find(normalizeHeaderKey(field));
    if (it != headers_.end())
    {
      result = it->second;
    }
 ...
 private:
  ...
  Version version_;
  string path_; // 请求路径
  string query_; // 请求方法 /user/login?username=tom&password=123 中的 username=tom&password=123
  string body_;
