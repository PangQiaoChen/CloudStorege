...
public:
    HttpUploadHandler(int numThreads, 
        ...
        , mysql(NULL)
    {
        threadPool_.start(numThreads); // 往线程池中创建线程并启动 runInThread 函数
...
private:
    ...
    bool handleFileUpload(const TcpConnectionPtr& conn, HttpRequest& req, HttpResponse* resp) {
        ...
        if (!uploadContext) { // 第一次请求，创建上传上下文
            ...
            std::string boundary = "--" + matches[1].str(); // 消息体的末尾边界格式为 --boundary
    ...
    bool handleListFiles(const TcpConnectionPtr& conn, HttpRequest& req, HttpResponse* resp) {
        ...
        if (listType == "my") {
            ...
            if (!escapedKeyword.empty()) {
                query += " AND f.original_filename LIKE '%" + escapedKeyword + "%'";
            }
            ...
            sendError(resp, "不支持的文件列表类型", HttpResponse::k400BadRequest, conn);
            return true;
        }
    ...
    bool handleDownload(const TcpConnectionPtr& conn, HttpRequest& req, HttpResponse* resp) {
        ...
        int fileId = std::stoi(row[0]); // 文件ID
        std::string serverFilename = row[1]; // 服务器端文件名
        std::string originalFilename = row[2]; // 原始文件名
        int fileOwnerId = std::stoi(row[3]); // 文件所有者ID
        std::string shareType = row[4] ? row[4] : ""; // 分享类型
        int sharedWithId = row[5] ? std::stoi(row[5]) : 0; // 分享给特定用户的ID
        std::string dbExtractCode = row[6] ? row[6] : ""; // 数据库中的提取码
        ...
        try {
            ...
            if (!downContext) {
                ...
                // 设置初始响应头
                if (isRangeRequest) { // 如果是范围请求，返回206 Partial Content
                ...
                resp->addHeader("Content-Length",
                              std::to_string(endPos - startPos + 1));
    ...
    bool handleLogin(const TcpConnectionPtr& conn, HttpRequest& req, HttpResponse* resp) {
        try {
            json requestData = json::parse(req.body()); // 解析JSON请求体
            std::string username = requestData["username"];
            std::string password = requestData["password"];
    ...
    void initRoutes() {
        // 不需要会话验证的路由
        addRoute("/favicon.ico", HttpRequest::kGet, &HttpUploadHandler::handleFavicon); // 网站的图标，通常显示在浏览器标签页上
        addRoute("/register", HttpRequest::kPost, &HttpUploadHandler::handleRegister); // 用户注册接口
        addRoute("/login", HttpRequest::kPost, &HttpUploadHandler::handleLogin); // 用户登录接口
        addRoute("/", HttpRequest::kGet, &HttpUploadHandler::handleIndex); // 网站首页，显示上传界面和文件列表
        addRoute("/index.html", HttpRequest::kGet, &HttpUploadHandler::handleIndex); // 网站首页
        addRoute("/register.html", HttpRequest::kGet, &HttpUploadHandler::handleIndex); // 注册页面，显示注册表单
        addRoute("/share/([^/]+)", HttpRequest::kGet, &HttpUploadHandler::handleShareAccess, {"code"}); // 通过分享码访问文件的接口
        addRoute("/share/download/([^/]+)", HttpRequest::kGet, &HttpUploadHandler::handleShareDownload, {"filename"}); // 通过分享链接下载文件的接口
        addRoute("/share/info/([^/]+)", HttpRequest::kGet, &HttpUploadHandler::handleShareInfo, {"code"}); // 获取分享链接信息的接口
        
        // 需要会话验证的路由
        addRoute("/upload", HttpRequest::kPost, &HttpUploadHandler::handleFileUpload); // 文件上传接口
        addRoute("/files", HttpRequest::kGet, &HttpUploadHandler::handleListFiles); // 列出用户文件的接口
        addRoute("/download/([^/]+)", HttpRequest::kHead, &HttpUploadHandler::handleDownload, {"filename"}); // 获取文件信息的接口（HEAD请求）

        addRoute("/download/([^/]+)", HttpRequest::kGet, &HttpUploadHandler::handleDownload, {"filename"}); // 下载文件的接口（GET请求）
        addRoute("/delete/([^/]+)", HttpRequest::kDelete, &HttpUploadHandler::handleDelete, {"filename"}); // 删除文件的接口
        addRoute("/share", HttpRequest::kPost, &HttpUploadHandler::handleShareFile); // 分享文件的接口
        addRoute("/users/search", HttpRequest::kGet, &HttpUploadHandler::handleSearchUsers); // 搜索用户的接口
        addRoute("/logout", HttpRequest::kPost, &HttpUploadHandler::handleLogout); // 用户登出的接口
    }
...
int main() {
    ...
    EventLoop loop;
    // 回调函数注册、创建并封装非阻塞 socket 【Acceptor】、bind()地址【Acceptor】
    HttpServer server(&loop, InetAddress(8000), "http-upload-test");
    
    // 创建HTTP处理器
    auto handler = std::make_shared<HttpUploadHandler>(4); // 业务工作线程池
    
    // 设置连接回调 : 告诉 server：如果有新连接来了，通知 handler 准备接待 (onConnection)
    server.setConnectionCallback(
        [handler](const TcpConnectionPtr& conn) {
            handler->onConnection(conn);
        });
    
    // 设置HTTP回调 : 告诉 server：如果有合法的 HTTP 请求发过来了，直接丢给 handler 里面的业务路由去处理 (onRequest)
    ...
    server.setThreadNum(0); // 设置所有 IO 都在baseLoop线程中进行【网络 IO 线程池】
    
    // 开始 listen()【Acceptor】，注册监听事件到 epoll 并将回调到底层的 EventLoop【Acceptor】
    server.start();
